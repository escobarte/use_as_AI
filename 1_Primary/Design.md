---
name: Truck Drivers Recruitment Landing Page
...
# Truck Drivers Recruitment Landing Page
...
```
"Connecting the Heart of the Supply Chain with the Careers They Deserve."
Subtitle: We bridge the gap between elite carriers and the professional drivers who keep the world moving, ensuring every mile driven is a mile valued.
```
## Website Goals:
**The goals should shift from "just getting clicks" to "getting the right person behind the wheel." Drivers are often browsing on mobile while at a rest stop or between loads, so efficiency is everything.**
```
1. Lead Generation (The "Primary" Goal)
The ultimate metric for success. You want the driver to take a specific action immediately.
* Quick App/Short Form: Capture the essentials (Name, Phone, CDL Class, Experience) before they lose interest.
* Phone Lead: Provide a "Click-to-Call" and "Apply Now" button front and center. Many drivers prefer talking to a recruiter over filling out forms.

2. Friction Reduction (User Experience)
Truckers have a high "bounce rate" if the site is slow or hard to use on a phone.

* Mobile-First Design: Ensure the site loads in under 3 seconds and is 100% usable with one thumb.
* Minimalist Navigation: Don’t distract them with a massive "About Us" section. Keep them focused on the job details and the application.

3. Building "Social Proof" & Trust
The trucking industry is full of "empty promises." Drivers are skeptical of recruiters.

* Transparency: Clearly display pay scales, home time, and equipment types.
* Driver Testimonials: Real photos and quotes from current drivers. Avoid stock photos if possible; drivers can spot a "fake trucker" from a mile away.
```
## Website Structure:
...
Sections in order:
1. **Hero Section** — Full-width truck image, headline, subheadline, CTA button → [links to Lid Form page]
2. **Pay & Benefits** — (Icons + numbers or Cards) (CPM rate, home time, bonuses) 
3. **Driver Requirements** — Checklist (CDL-A, experience, etc.)
4. **Modern Fleet** — Truck photo, fleet description
5. **Driver Testimonials** - Short video clips or quotes from actual drivers. || Safety Awards/Ratings: Any industry certifications or high CSA scores.
6. **Why Drive With Us** — 3–4 cards with icons, short text
7. **Final CTA** — Big button → [links to Lid Form page]
8. **Footer** — Logo, address, MC#, USDOT#, phone, email, Contact Us (button to send email)

### Page 2: Lid Form Page
Sections in order:
